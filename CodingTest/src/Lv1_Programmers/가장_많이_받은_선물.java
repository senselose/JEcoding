...
public class 가장_많이_받은_선물 {
	...
	public static void main(String[] args) {
        ...
        // 선물 주고 받은 2차원 배열 생성
        int[][] giftRecode = new int[friends.length][friends.length];
        
		// 선물 지수 증감 및 2차원 배열에 횟수 넣기
        for(int i=0; i<gifts.length; i++) {
        	String[] gift = gifts[i].split(" ");
        	giftIndex[map.get(gift[0])]++; // 선물한 사람 선물지수 ++
        	giftIndex[map.get(gift[1])]--; // 선물 받은사람 선물지수 --
        	giftRecode[map.get(gift[0])][map.get(gift[1])]++; // 선물 주고 받은 내역 ++ A -> B
        }
		
        // 확인
        System.out.println("친구들 간의 선물 주고받은 횟수 (giftRecode):");
        for (int i = 0; i < giftRecode.length; i++) {
            for (int j = 0; j < giftRecode[i].length; j++) {
                System.out.print(giftRecode[i][j] + " ");
            }
            System.out.println();
        }
        ...
        for(int i=0; i<friends.length; i++) {
        	...
        	for(int j=0; j<friends.length; j++) {
        		...
        		// 선물 준게 선물 받은거보다 많으면 받을 선물 ++
        		if(giftRecode[i][j] > giftRecode[j][i]) {
        			count ++;
        		}
        		// 선물 주고받은게 같지만 giftIndex가 높으면 선물 받을 갯수 ++
        		else if(giftRecode[i][j] == giftRecode[j][i] && giftIndex[i] > giftIndex[j]) {
        			count++;
        		}
        		// count 최대값 answer에 저장
        		answer = Math.max(answer, count);
        	}
        }
        
        System.out.println("선물 받는 최대 선물 갯수: " + answer);
